...
class PatternOutcomeAnalyzer:
    ...
    def start_monitoring(self, pattern_records: List[PatternRecord]):
        ...
        for pattern in pattern_records:
            if pattern.id not in self.monitoring_patterns:
                ...
                logger.info(f"开始监控形态 {pattern.id} ({pattern.sub_type.value if pattern.sub_type else pattern.pattern_type})")
    ...
    def update_monitoring(self, df: pd.DataFrame) -> List[PatternOutcomeAnalysis]:
        ...
        for pattern_id, pattern in self.monitoring_patterns.items():
            ...
            if self._is_monitoring_timeout(pattern, current_time):
                ...
                logger.info(f"形态 {pattern_id} 监控超时，结局: {analysis.outcome.value}")
                continue
            ...
            if analysis and analysis.outcome != PatternOutcome.MONITORING:
                ...
                logger.info(f"形态 {pattern_id} 结局确定: {analysis.outcome.value}")
    ...
    def _is_monitoring_timeout(self, pattern: PatternRecord, current_time: datetime) -> bool:
        ...
        if pattern.pattern_boundaries:
            ...
            pattern_end_time = pattern.flagpole.end_time
        ...
        # 根据时间周期估算间隔（这里简化处理）
        # 实际应该从数据中推导时间间隔
        time_interval_minutes = self._estimate_timeframe_interval(pattern.symbol)
        timeout_time = pattern_end_time + timedelta(minutes=time_interval_minutes * timeout_duration)
        
        return current_time >= timeout_time
    
    def _estimate_timeframe_interval(self, symbol: str) -> int:
        """
        根据symbol估算时间周期间隔（分钟）
        这是一个简化实现，实际应该从数据中推导
        """
        # 从symbol中提取时间周期信息，或使用默认值
        if '15min' in symbol:
            return 15
        elif '5min' in symbol:
            return 5
            ...
            'monitoring_patterns': [
                {
                    'id': p.id,
                    'symbol': p.symbol,
                    'sub_type': p.sub_type.value if p.sub_type else p.pattern_type,
