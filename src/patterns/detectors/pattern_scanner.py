...
from src.patterns.base.market_regime_detector import (
    RegimeDetector, BaselineManager
)
from src.patterns.detectors.flagpole_detector import FlagpoleDetector
from src.patterns.detectors.flag_pattern_detector import FlagPatternDetector
...
class PatternScanner:
    ...
    def _initialize_components(self):
        ...
        # 阶段2：旗面检测器
        self.flag_detector = FlagPatternDetector(self.baseline_manager)
    ...
    def scan(self, df: pd.DataFrame, 
        ...
        logger.info(f"Starting pattern scan on {len(df)} data points")
    ...
    def _validate_input_data(self, df: pd.DataFrame) -> bool:
        ...
        if len(df) < min_data_points:
            ...
            logger.warning("  1. Using a larger date range")
            logger.warning("  2. Reducing min_data_points in config")
            logger.warning("  3. Using a different data source with more historical data")
            if len(df) > 0:
                logger.warning(f"  Available data range: {df['timestamp'].min()} to {df['timestamp'].max()}")
            return False
