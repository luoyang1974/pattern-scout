...
class SimpleFlagpoleVisualizer:
    """简化的旗杆可视化器"""
    ...
    def __init__(self):
        ...
        # 输出目录
        self.output_dir = Path(project_root) / "output" / "flagpole_tests" / "simple_all_flagpoles"
    ...
    def run_main_program_detection(self, symbol: str = "RBL8"):
        """运行主程序进行旗杆检测"""
        print("\\n调用主程序进行旗杆检测...")
        
        try:
            # 导入主程序
            from main import PatternScout
            ...
            results = scout.run(
                ...
                generate_charts=False,  # 不生成默认图表，我们自己生成
                generate_baseline_summary=True,
                generate_outcome_charts=False
            )
            
            print(f"主程序检测完成，结果: {results}")
            return results
            
        except Exception as e:
            print(f"运行主程序检测失败: {str(e)}")
            import traceback
            traceback.print_exc()
            return None
    
    def load_detected_patterns(self) -> List[Dict]:
        """从输出目录加载检测到的形态数据"""
        patterns = []
        
        try:
            # 查找 patterns 目录下的 JSON 文件
            patterns_dir = Path(project_root) / "output" / "data" / "patterns"
            
            if not patterns_dir.exists():
                print(f"形态数据目录不存在: {patterns_dir}")
                return []
            
            # 查找 RBL8 相关的 JSON 文件
            json_files = list(patterns_dir.glob("*RBL8*.json"))
            print(f"找到 {len(json_files)} 个形态数据文件")
            ...
            for json_file in json_files:
                try:
                    ...
                    print(f"从 {json_file.name} 加载了形态数据")
                    
                except Exception as e:
                    print(f"加载文件 {json_file} 失败: {e}")
                    continue
            
            print(f"总共加载了 {len(patterns)} 个形态")
            return patterns
            
        except Exception as e:
            print(f"加载形态数据失败: {str(e)}")
            return []
    
    def extract_flagpoles_from_patterns(self, patterns: List[Dict]) -> List[Dict]:
        """从形态数据中提取旗杆信息"""
        flagpoles = []
        
        try:
            for pattern in patterns:
                if pattern.get('pattern_type') == 'flag_pattern':
                    # 提取旗杆信息
                    flagpole_data = pattern.get('flagpole', {})
                    
                    if flagpole_data:
                        flagpole = {
                            'start_idx': flagpole_data.get('start_index', 0),
                            'end_idx': flagpole_data.get('end_index', 0),
                            'direction': flagpole_data.get('direction', 'up'),
                            'height_pct': flagpole_data.get('height_percentage', 0),
                            'slope_score': flagpole_data.get('slope', 0.5),
                            'volume_burst': flagpole_data.get('volume_burst_ratio', 1.5),
                            'volume_spike': flagpole_data.get('max_volume_ratio', 2.0),
                            'retrace_pct': flagpole_data.get('max_retracement_percentage', 10.0),
                            'volatility': flagpole_data.get('atr_normalized_height', 1.0),
                            'quality_score': pattern.get('confidence', 0.5),
                            'start_price': flagpole_data.get('start_price', 0),
                            'end_price': flagpole_data.get('end_price', 0),
                            'length': flagpole_data.get('length', 5),
                            'detection_params': {
                                'min_height_pct': 0.5,
                                'min_slope_score': 0.1,
            ...
            print(f"从 {len(patterns)} 个形态中提取了 {len(flagpoles)} 个旗杆")
            return flagpoles
            
        except Exception as e:
            print(f"提取旗杆信息失败: {str(e)}")
            return []
    ...
    def run_complete_workflow(self, symbol: str = "RBL8") -> Dict[str, Any]:
        """运行完整的工作流程"""
        print("\\n开始完整旗杆检测和可视化工作流...")
        
        # 1. 运行主程序检测
        main_results = self.run_main_program_detection(symbol)
        if not main_results:
            print("主程序检测失败")
            return {}
        ...
        # 3. 加载检测到的形态
        patterns = self.load_detected_patterns()
        if not patterns:
            print("未找到检测到的形态数据")
            return {}
        
        # 4. 提取旗杆信息
        flagpoles = self.extract_flagpoles_from_patterns(patterns)
        if not flagpoles:
            print("未找到旗杆信息")
            return {}
    ...
    def _generate_complete_report(self, results: Dict[str, Any], flagpoles: List[Dict]):
        ...
        try:
            ...
            with open(report_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"**检测方式**: 主程序系统检测器 + 数据去重优化\\n\\n")
                ...
                f.write("## 系统优势\\n\\n")
                f.write("- **数据去重**: 自动去除重复记录，提高数据质量\\n")
                f.write("- **主程序检测**: 使用完整的六阶段动态基线系统\\n")
                f.write("- **高效处理**: 处理完整时间范围内的所有数据\\n")
                f.write("- **专业可视化**: 标准化图表格式，详细参数标注\\n\\n")
...
def main():
    """主函数"""
    try:
        visualizer = SimpleFlagpoleVisualizer()
        
        # 运行完整工作流程
        results = visualizer.run_complete_workflow("RBL8")
