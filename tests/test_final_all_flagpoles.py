...
class FinalFlagpoleDetector:
    """最终版旗杆检测器 - 使用优化的放宽阈值"""
    
    def __init__(self):
        # 使用非常宽松的阈值以检测更多旗杆
        self.detection_params = {
            'min_height_pct': 0.3,       # 最小高度 0.3%
            'min_slope_score': 0.05,     # 极低斜率要求
            'min_volume_burst': 1.1,     # 极低量能要求
            'max_retrace_pct': 60,       # 允许更大回撤
            'min_length': 2,             # 最小长度2周期
            'max_length': 12,            # 最大长度12周期
    ...
    def detect_flagpoles(self, df: pd.DataFrame) -> List[Dict]:
        ...
        progress_step = max(1, total_iterations // 50)  # 每2%显示进度
        ...
        for i in range(self.detection_params['context_period'], 
            ...
            for length in range(self.detection_params['min_length'], 
                ...
                volume_burst = pole_volume / base_volume if base_volume > 0 else 1.5
                ...
                # 计算质量得分 - 放宽权重
                quality_score = (height_pct * 0.5 + slope_score * 0.2 + 
                               min(volume_burst, 3) * 0.3) / 3
                
                # 检查重叠 - 允许更多重叠
                overlap = False
                for existing in flagpoles:
                    overlap_threshold = 0.7  # 70%重叠才视为冲突
                    ...
                    if overlap_end > overlap_start:
                        ...
                        overlap_ratio = overlap_length / min(current_length, existing_length)
...
class FinalFlagpoleVisualizer:
    ...
    def __init__(self):
        ...
        # 输出目录
        self.output_dir = Path(project_root) / "output" / "flagpole_tests" / "final_all_flagpoles"
        self.output_dir.mkdir(parents=True, exist_ok=True)
        
        # 旗杆检测器
        self.detector = FinalFlagpoleDetector()
        
        print("最终版旗杆可视化器初始化完成")
    ...
    def run_final_detection_and_visualization(self, symbol: str = "RBL8") -> Dict[str, Any]:
        """运行最终的旗杆检测和可视化"""
        print("\\n开始最终版旗杆检测和可视化...")
        ...
        for i, flagpole in enumerate(flagpoles):
            ...
            # 显示进度
            if i % 20 == 0 or i == len(flagpoles) - 1:
        ...
        # 生成汇总报告
        self._generate_final_report(results, flagpoles)
    ...
    def _generate_final_report(self, results: Dict[str, Any], flagpoles: List[Dict]):
        """生成最终的汇总报告"""
        try:
            report_path = self.output_dir / f"final_flagpoles_report.md"
            
            with open(report_path, 'w', encoding='utf-8') as f:
                f.write(f"# {results['data_info']['symbol']} - 最终版完整旗杆检测和可视化报告\\n\\n")
                f.write(f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\\n")
                f.write(f"**检测方式**: 优化检测器 + 放宽阈值 + 数据去重优化\\n\\n")
                ...
                if flagpoles:
                    ...
                    f.write(f"- **最小高度**: {params['min_height_pct']}% (极低阈值)\\n")
                    f.write(f"- **最小斜率**: {params['min_slope_score']} (极低要求)\\n")
                    f.write(f"- **最小量能爆发**: {params['min_volume_burst']}x (极低要求)\\n")
                    f.write(f"- **最大回撤**: {params['max_retrace_pct']}% (允许更大回撤)\\n")
                    f.write(f"- **长度范围**: {params['min_length']}-{params['max_length']} 周期 (更宽范围)\\n")
                    f.write(f"- **重叠容忍**: 70% 重叠阈值 (允许更多形态)\\n\\n")
                
                f.write("## 系统优势\\n\\n")
                f.write("- **数据去重**: 自动去除 1,110 条重复记录\\n")
                f.write("- **放宽阈值**: 使用极低的检测阈值，发现更多形态\\n")
                f.write("- **完整覆盖**: 处理 2019-2025 年完整时间范围\\n")
                f.write("- **专业可视化**: 标准化图表，详细参数标注\\n")
                f.write("- **智能重叠**: 70%重叠容忍度，保留更多有效形态\\n\\n")
...
def main():
    """主函数"""
    try:
        visualizer = FinalFlagpoleVisualizer()
        
        # 运行最终版检测和可视化
        results = visualizer.run_final_detection_and_visualization("RBL8")
